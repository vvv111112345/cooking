...
// Проверка и обновление статуса избранного после AJAX-запроса
if (isset($_GET['favorite_updated'])) {
    $isFavorite = $pdo->prepare("SELECT * FROM favorites WHERE user_id = ? AND recipe_id = ?")
...
function toggleFavorite(recipeId, isFavorite) {
    ...
    .then(message => {
        ...
        // Если добавляли в избранное - обновляем страницу через 1 секунду
        if (!isFavorite) {
            setTimeout(() => {
