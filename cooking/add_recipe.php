...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        $fileName = 'recipe_' . uniqid() . '.' . $fileExt;
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO recipes 
            (category_id, recipe_name, description, instructions, prep_time, cook_time, servings, user_id, image_url) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $categoryId, $recipeName, $description, $instructions, 
            $prepTime, $cookTime, $servings, $userId, $imageUrl
        ]);
        ...
        if (!empty($_POST['ingredients'])) {
            foreach ($_POST['ingredients'] as $index => $ingredientName) {
                $quantity = $_POST['quantities'][$index] ?? 0;
                ...
                if (!$ingredient) {
                    ...
                    $ingredientStmt = $pdo->prepare("
                        ...
                        VALUES (?, ?)
                    ");
                    $ingredientStmt->execute([$ingredientName, $unit]);
                ...
                $recipeIngredientStmt = $pdo->prepare("
                    INSERT INTO recipe_ingredients 
                    (recipe_id, ingredient_id, quantity) 
                    VALUES (?, ?, ?)
                ");
                $recipeIngredientStmt->execute([$recipeId, $ingredientId, $quantity]);
            }
        }
        ...
        $pdo->rollBack();
        die("Ошибка при сохранении рецепта: " . $e->getMessage());
    }
}
?>
<!DOCTYPE html>
<!-- Остальная часть HTML-кода остается без изменений -->
<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
